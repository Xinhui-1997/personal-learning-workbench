...
function renderProgress(){const cols=`repeat(${MODULES.length},1fr)`;$('segments').style.gridTemplateColumns=cols;$('segmentLabels').style.gridTemplateColumns=cols;$('segments').innerHTML=MODULES.map(m=>`<div class="segment ${m.id} ${completion[m.id]?'done':''}"></div>`).join('');$('segmentLabels').innerHTML=MODULES.map(m=>`<span>${m.label}</span>`).join('');$('progressText').textContent=`今日完成 ${MODULES.filter(m=>completion[m.id]).length} / ${MODULES.length}`}
function renderCards(){if(!cards.length){$('cards').innerHTML=`<div class="empty"><div class="emoji">🗓️</div><strong>${showDate(currentDate)}</strong><br>这一天还没有内容。</div>`;return}const by=Object.fromEntries(cards.map(c=>[c.module,c]));$('cards').innerHTML=MODULES.filter(m=>by[m.id]).map(m=>card(m,by[m.id])).join('');document.querySelectorAll('[data-complete]').forEach(b=>b.onclick=()=>toggleComplete(b.dataset.complete))}
